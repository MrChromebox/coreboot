...
#
set -e
filename="coreboot_seabios-gandof-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-gandof-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-gandof .config
make
