...
#
set -e
filename="coreboot_seabios-leon-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-leon-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-leon .config
make
