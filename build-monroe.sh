...
#
set -e
filename="coreboot_seabios-monroe-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-monroe-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-monroe .config
make
