...
#
set -e
filename="coreboot_seabios-ninja-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-ninja .config
make
...
cbfstool ${filename} add-int -i 0xd081f000 -n etc/sdcard0
cbfstool ${filename} add-int -i 0xd081c000 -n etc/sdcard1
