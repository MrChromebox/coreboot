...
#
set -e
filename="coreboot_seabios-rikku-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-rikku-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -f ~/firmware/${filename2}*
rm -rf ./build
cp configs/.config-rikku .config
make
if [ "$1" != "uefi" ]; then
