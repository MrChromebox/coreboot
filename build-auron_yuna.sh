...
#
set -e
filename="coreboot_seabios-auron_yuna-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-auron_yuna-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
# build auron_paine
rm -rf ./build
cp configs/.config-auron_yuna .config
make
