...
#
set -e
filename="coreboot_seabios-wolf-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-wolf-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-wolf .config
make
