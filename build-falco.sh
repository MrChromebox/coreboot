...
#
set -e
filename="coreboot_seabios-falco-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-falco-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-falco .config
make
