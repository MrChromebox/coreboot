...
#
set -e
filename="coreboot_seabios-lulu-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-lulu-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-lulu .config
make
