...
#
set -e
filename="coreboot_seabios-auron_paine-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-auron_paine-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-auron_paine .config
make
...
cbfstool ${filename2} add -f ./duet-hswbdw-book.img -n floppyimg/tianocore.img.lzma -t raw -c lzma
