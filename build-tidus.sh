...
#
set -e
filename="coreboot_seabios-tidus-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-tidus-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -f ~/firmware/${filename2}*
rm -rf ./build
cp configs/.config-tidus .config
make
if [ "$1" != "uefi" ]; then
