...
#
set -e
filename="coreboot_seabios-samus-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-samus-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -rf ./build
cp configs/.config-samus .config
make
