#!/bin/bash
#
filename="coreboot-seabios-stumpy-`date +"%Y%m%d"`-md.rom"
rm -f ${filename}*
rm -rf ./build
cp .config-stumpy .config
make
if [ $? -eq 0 ]; then
dd if=./3rdparty/mainboard/samsung/stumpy/descriptor.bin of=./build/coreboot.rom conv=notrunc
...
cbfstool ${filename} add -f ./cbfs/links-stumpy -n links -t raw
