...
#
set -e
filename="coreboot_seabios-panther-mrchromebox_`date +"%Y%m%d"`.rom"
filename2="coreboot_seabios_duet-panther-mrchromebox_`date +"%Y%m%d"`.rom"
rm -f ~/firmware/${filename}*
rm -f ~/firmware/${filename2}*
rm -rf ./build
cp configs/.config-panther .config
make
if [ "$1" != "uefi" ]; then
